...
public class NewHomeActivity extends NexusLauncherActivity {
    ...
    public void installAssetApk(String apkpath){
        File apk = getFileStreamPath(apkpath);
        if(!apk.exists()){
            InputStream input = null;
            OutputStream output = null;
            try {
                input = getApplicationContext().getAssets().open(apkpath);
        ...
        if (apk.isFile() && !DeviceUtil.isMeizuBelowN()) {
            try {
                VirtualCore.get().installPackage(apk.getPath(), InstallStrategy.TERMINATE_IF_EXIST);
            } catch (Throwable e) {
                VLog.e(TAG, "installPackage", e);
            }
        }
